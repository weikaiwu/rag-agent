...
def step_6_deal_list(state,item_results, history_chats,rewritten_query):
    ...
    if len(options_item_names) > 0:
        ...
        # 给出可选主题提示，但仍放行进入全局检索（不阻断流程）
        logger.info(f"有可选的item_name:{options_item_names}，将进行全局语义检索")
    # 4. 确认和可选集合都没数据 （处理）
    # 不再阻断：直接放行进入全局语义检索，item_names 置空以触发全库检索
    logger.info(f"未匹配到明确主题，将进行全局语义检索")
    state['item_names'] = []
    state['rewritten_query'] = rewritten_query
    state['history'] = history_chats
    if "answer" in state:
...
def node_item_name_confirm(state):
    ...
    # 响应数据： item_names: List[str]  # 提取出的知识主题名称
    ...
    #  响应： { item_names : [RAG技术文档] , rewritten_query : str }
    #  1. 为啥问题要重写？
    """
       1. 消除指代歧义    他 Ta 它 不明确！  明确查询主体 item_name
       2. 补全上下文     他的问题需要有历史记录支持！ 
       3. 去掉口语和冗余  （如口语化、方言、非正式缩略表达）
    ...
    if len(item_names) > 0 :
        # 向量数据库查询 item_name
        # 4. milvus向量查询 item_names -》 模型提取 不一定跟我们向量数据库的完全相同(技术文档主题)
