...
def save_chat_message(
    ...
    :param item_names: 关联的文档主题列表（可选，支持多主题，默认None）
    ...
    document = {
        ...
        "item_names": item_names,  # 关联文档主题列表
...
def update_message_item_names(ids: List[str], item_names: List[str]) -> int:
    ...
    try:
        ...
        result = mongo_tool.chat_message.update_many(
            ...
            {"$set": {"item_names": item_names}}  # 更新操作：设置新的文档主题列表
        )
        # 记录更新成功日志，包含更新数量和新的文档主题
...
if __name__ == "__main__":
    ...
    # 3. 写入带关联主题的文档消息（手动指定ts=1002，测试item_names字段）
    save_chat_message(sid, "user", "什么是混合检索中的稀疏向量表示？", item_names=["混合检索"])
